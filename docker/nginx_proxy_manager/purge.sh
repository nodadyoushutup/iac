...
# Removes Docker assets that belong to the Nginx Proxy Manager deployment
# (services, configs, secrets, containers, networks, images) without touching
# unrelated resources.
...
remove_items() {
  ...
  echo "==> Removing ${APP_NAME^} ${description}..."
...
purge_node_assets() {
  ...
  if ! remove_items "containers on ${node_label}" \
    docker ps -aq --filter name="${APP_NAME}" \
    :: docker rm -f
  then
    had_error=1
  fi
  ...
  if ! remove_items "volumes on ${node_label}" \
    docker volume ls --filter name="${APP_NAME}" --format '{{.Name}}' \
    :: docker volume rm -f
  then
    had_error=1
  fi
  ...
  if ! remove_items "networks on ${node_label}" \
    docker network ls --filter name="${APP_NAME}" --format '{{.ID}}' \
    :: docker network rm
  then
    had_error=1
  fi
  ...
  if ! remove_items "images on ${node_label}" \
    docker image ls --filter "reference=*${APP_NAME}*" --quiet \
    :: docker image rm -f
  then
    had_error=1
  fi
...
ensure_known_host() {
  ...
  if ssh-keyscan "${keyscan_args[@]}" >"${tmpfile}" 2>/dev/null && [[ -s "${tmpfile}" ]]; then
    ...
    echo "    warning: failed to auto-trust ${host}; continuing without modification." >&2
  fi
...
purge_remote_node() {
  ...
  for attempt_host in "${host_candidates[@]}"; do
    ...
    echo "==> Purging ${APP_NAME^} assets on node ${node_hostname} (${attempt_host})..."
...
echo "==> Purging ${APP_NAME^} Docker assets..."
...
if [[ "${manager_ops_available}" == "true" ]]; then
  if ! remove_items "stacks" \
    bash -lc "docker stack ls --format '{{.Name}}' | awk '/${APP_NAME}/'" \
    :: docker stack rm
  then
  ...
  if ! remove_items "services" \
    docker service ls --filter name="${APP_NAME}" --format '{{.ID}}' \
    :: docker service rm
  then
  ...
  if ! remove_items "configs" \
    docker config ls --filter name="${APP_NAME}" --format '{{.ID}}' \
    :: docker config rm
  then
  ...
  if ! remove_items "secrets" \
    docker secret ls --filter name="${APP_NAME}" --format '{{.ID}}' \
    :: docker secret rm
  then
...
echo "==> ${APP_NAME^} Docker assets purge complete."
