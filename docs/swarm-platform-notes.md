...
- Renovate platform selection is handled via self-hosted bot configs under `docker/state/renovate-*.json` (arm bot sets `docker.platform=linux/arm64`). Repo-level `.github/renovate.json` does not set platform to avoid config warnings.
