output "_validate_results" {
  value = var.FLAG_PYVENV == 1 ? data.external.validate_env[0].result["valid"] != "false" ? "Deploment ${var.FLAG_DEPLOY} - Validation checks have passed" : "Deploment ${var.FLAG_DEPLOY} - Some validation checks failed" : null
}
