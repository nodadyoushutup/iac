module "cloud_required" {
  ...
  for_each = {
    for idx, machine in local.machine.cloud.required :
    try(machine.name, null) != null ? machine.name : "default_${idx}" => machine
  }

  cloud_config = each.value.cloud_config.auth.github != null ? each.value.cloud_config : local.machine.global.cloud_config
...
# module "cloud_custom" {
#   source = "../../module/proxmox/virtual_machine"
#   for_each = { for machine in local.machine.cloud.custom : machine.name => machine }
