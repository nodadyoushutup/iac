...
resource "proxmox_virtual_environment_vm" "talos_cp_0" {
  ...
  disk {
    datastore_id = "local-lvm"
    file_id      = proxmox_virtual_environment_download_file.talos_cloud_image_ip_210.id
    interface    = "scsi0"
    discard      = "on"
    size         = 20
  }
...
resource "proxmox_virtual_environment_download_file" "talos_cloud_image_ip_210" {
  content_type = "iso"
  datastore_id = "local"
  node_name    = "pve"
  file_name = "talos_cloud_image_ip_210.img"
  url = "https://factory.talos.dev/image/1ea23916cd08c79a19a004078f2449a181fbdb16df684122b5f9c73cb3e53e13/v1.8.3/metal-amd64.qcow2"
}
