...
resource "talos_machine_configuration_apply" "controlplane" {
  ...
  config_patches = [
    templatefile("${path.module}/templates/install-disk-and-hostname.yaml.tmpl", {
      hostname     = each.value.hostname == null ? format("%s-cp-%s", var.cluster_name, index(keys(local.node_data.controlplane), each.key)) : each.value.hostname
...
resource "talos_machine_configuration_apply" "worker" {
  ...
  config_patches = [
    templatefile("${path.module}/templates/install-disk-and-hostname.yaml.tmpl", {
      hostname     = each.value.hostname == null ? format("%s-worker-%s", var.cluster_name, index(keys(local.node_data.worker), each.key)) : each.value.hostname
      install_disk = each.value.install_disk
    })
  ]
}
