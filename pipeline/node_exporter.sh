...
TFVARS_ARG=""
BACKEND_ARG=""
DEFAULT_TFVARS_BASENAME="node_exporter"
DEFAULT_TFVARS_FILE="${HOME}/.tfvars/node_exporter.tfvars"
DEFAULT_BACKEND_FILE="${HOME}/.tfvars/minio.backend.hcl"

usage() {
  cat <<'USAGE'
Usage: pipeline/node_exporter.sh [--tfvars <path>] [--backend <path>] [tfvars_path] [backend_path]

Runs a multi-stage Terraform workflow:
  1) Plans and applies the node_exporter application infrastructure (module.node_exporter_app)
  2) Plans and applies the node_exporter configuration layer
