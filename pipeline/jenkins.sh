...
TERRAFORM_DIR="${ROOT_DIR}/terraform/swarm/jenkins"
...
TFVARS_ARG=""
BACKEND_ARG=""
DEFAULT_TFVARS_BASENAME="jenkins"
DEFAULT_TFVARS_FILE="${HOME}/.tfvars/jenkins.tfvars"
...
Runs a multi-stage Terraform workflow:
  ...
  2) Plans and applies the Jenkins configuration layer

Optional arguments can be provided either as flags or positional values.
If omitted, defaults are:
  TFVARS  -> $HOME/.tfvars/jenkins.tfvars (falls back to first *.tfvars in ./terraform/swarm/jenkins)
...
echo "[STAGE] Jenkins app plan"
if ! "${EXEC_SCRIPT}" plan -input=false -refresh=false -var-file="${TFVARS_PATH}" -target=module.jenkins_app; then
  echo "[ERR] terraform plan (app) failed" >&2
  exit 1
fi

echo "[STAGE] Jenkins app apply"
if ! "${EXEC_SCRIPT}" apply -input=false -refresh=false -auto-approve -var-file="${TFVARS_PATH}" -target=module.jenkins_app; then
...
if ! "${EXEC_SCRIPT}" plan -input=false -var-file="${TFVARS_PATH}"; then
  echo "[ERR] terraform plan (config) failed" >&2
  exit 1
fi

echo "[STAGE] Jenkins config apply"
if ! "${EXEC_SCRIPT}" apply -input=false -auto-approve -var-file="${TFVARS_PATH}"; then
  echo "[ERR] terraform apply (config) failed" >&2
  exit 1
fi

echo "[DONE] Jenkins app and configuration applied."
