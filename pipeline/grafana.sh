...
TERRAFORM_DIR="${ROOT_DIR}/terraform/swarm/grafana"
...
TFVARS_ARG=""
BACKEND_ARG=""
DEFAULT_TFVARS_BASENAME="grafana"
DEFAULT_TFVARS_FILE="${HOME}/.tfvars/grafana.tfvars"
DEFAULT_BACKEND_FILE="${HOME}/.tfvars/minio.backend.hcl"

usage() {
  cat <<'USAGE'
Usage: pipeline/grafana.sh [--tfvars <path>] [--backend <path>] [tfvars_path] [backend_path]

Runs a multi-stage Terraform workflow:
  1) Plans and applies the Grafana application infrastructure (module.grafana_app)
  2) Plans and applies the full Grafana stack (app + config)

Optional arguments can be provided either as flags or positional values.
If omitted, defaults are:
  TFVARS  -> $HOME/.tfvars/grafana.tfvars (falls back to first *.tfvars in ./terraform/swarm/grafana)
...
echo "[STAGE] Grafana app plan"
if ! "${EXEC_SCRIPT}" plan -input=false -refresh=false -var-file="${TFVARS_PATH}" -target=module.grafana_app; then
  echo "[ERR] terraform plan (app) failed" >&2
  exit 1
fi

echo "[STAGE] Grafana app apply"
if ! "${EXEC_SCRIPT}" apply -input=false -refresh=false -auto-approve -var-file="${TFVARS_PATH}" -target=module.grafana_app; then
  echo "[ERR] terraform apply (app) failed" >&2
  exit 1
fi

echo "[STAGE] Grafana stack plan"
if ! "${EXEC_SCRIPT}" plan -input=false -var-file="${TFVARS_PATH}"; then
  echo "[ERR] terraform plan (stack) failed" >&2
  exit 1
fi

echo "[STAGE] Grafana stack apply"
if ! "${EXEC_SCRIPT}" apply -input=false -auto-approve -var-file="${TFVARS_PATH}"; then
  echo "[ERR] terraform apply (stack) failed" >&2
  exit 1
fi

echo "[DONE] Grafana app and configuration applied."
