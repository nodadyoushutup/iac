...
TFVARS_ARG=""
BACKEND_ARG=""
DEFAULT_TFVARS_BASENAME="dozzle"
DEFAULT_TFVARS_FILE="${HOME}/.tfvars/dozzle.tfvars"
DEFAULT_BACKEND_FILE="${HOME}/.tfvars/minio.backend.hcl"

usage() {
  cat <<'USAGE'
Usage: pipeline/dozzle.sh [--tfvars <path>] [--backend <path>] [tfvars_path] [backend_path]

Runs a multi-stage Terraform workflow:
  1) Plans and applies the dozzle application infrastructure (module.dozzle_app)
  2) Plans and applies the dozzle configuration layer
