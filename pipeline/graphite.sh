...
TERRAFORM_DIR="${ROOT_DIR}/terraform/swarm/graphite"
...
TFVARS_ARG=""
BACKEND_ARG=""
DEFAULT_TFVARS_BASENAME="graphite"
DEFAULT_TFVARS_FILE="${HOME}/.tfvars/graphite.tfvars"
DEFAULT_BACKEND_FILE="${HOME}/.tfvars/minio.backend.hcl"

usage() {
  cat <<'USAGE'
Usage: pipeline/graphite.sh [--tfvars <path>] [--backend <path>] [tfvars_path] [backend_path]

Runs the Graphite Terraform workflow (single module containing app + config).

Optional arguments can be provided either as flags or positional values.
If omitted, defaults are:
  TFVARS  -> $HOME/.tfvars/graphite.tfvars (falls back to first *.tfvars in ./terraform/swarm/graphite)
